...
namespace PageRank
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // If there is no input file, a path can be written in
            System.IO.StreamReader inputFile;
            if (!(args.Length > 0))
            {
                Console.WriteLine("No input file detected. Input File address: ");
                var path = Console.ReadLine();
                inputFile = new System.IO.StreamReader(path);
            }
            else
            {
